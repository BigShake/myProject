...
<template>
    <div>
        <div class="title-content-block">
            <div>
                <h1 style="font-size:15px;">同步用法</h1>
            </div>
        </div>
        <div>
            <div style="padding: 20px 0 5px 0">
                <Button style="margin-right: 20px;" @click="syncBtnClick()">同步用法</Button>
...
export default {
    ...
    methods: {
        ...
        syncBtnClick() {
            this.$Modal.confirm(this.modelConfig);
        ...
        syncUseage() {
            let _this = this; 
            _this.spinShow = true;
            util.request(global.API_PREFIX + "/useageSync/synchronization"
            ).then(function(response) {
                if (response.data.code === 1) {
