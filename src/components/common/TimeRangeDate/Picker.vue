...
<script>
    ...
    import { formatDate, parseDate } from 'iview/src/components/date-picker/util';
        ...
        computed: {
            ...
            visualValue: {
                get () {
                    ...
                    let result = formatter(value, this.format || format);
                    return result + " " + this.currentDateRange;
                },
        ...
        methods: {
            ...
            handleInputChange (event) {
                ...
                if (correctValue !== oldValue) {
                    this.emitChange(correctDate, this.currentDateRange);
                }
            },
            ...
            showPicker () {
                if (!this.picker) {
                    ...
                    if (this.disabledHours) {
                        this.picker.disabledHours = this.disabledHours;
                    }
                    if (this.disabledMinutes) {
                        this.picker.disabledMinutes = this.disabledMinutes;
                    }
                    if (this.disabledSeconds) {
                        this.picker.disabledSeconds = this.disabledSeconds;
                    }
                    if (this.hideDisabledOptions) {
                        this.picker.hideDisabledOptions = this.hideDisabledOptions;
                    }
                    ...
                    this.picker.$on('on-pick', (date, dateRange, visible = false) => {
                        if (!isConfirm) {
                        ...
                        this.emitChange(date, dateRange);
                    });
            ...
            emitChange (date, dateRange = "") {
                const newDate = this.formattingDate(date);

                this.$emit('on-change', newDate + dateRange);
                this.$nextTick(() => {
                    this.dispatch('FormItem', 'on-form-change', newDate + dateRange);
                });
            },
            formattingDate (date) {
                const type = this.type;
        ...
        watch: {
            visible (val) {
                if (val) {
                    ...
                    if (this.picker) {
                        this.picker.resetView && this.picker.resetView(true);
                    }
