<template>
    <div :class="classes">
        ...
        <div :class="[prefixCls + '-body']">
            ...
            <div style="" class="ivu-picker-confirm">
                <Radio-group v-model="dateRange" @on-change="handleDateRangePick">
                    <Radio label="0">
                        <span>上午</span>
                    </Radio>
                    <Radio label="1">
...
<script>
    ...
    import DateTable from 'iview/src/components/date-picker/base/date-table.vue';
    ...
    import TimePicker from 'iview/src/components/date-picker/panel/time.vue';
    import Confirm from 'iview/src/components/date-picker/base/confirm.vue';

    import Mixin from 'iview/src/components/date-picker/panel/mixin';
    import Locale from 'iview/src/mixins/locale';

    import { initTimeDate } from 'iview/src/components/date-picker/util';
    ...
    export default {
        ...
        watch: {
            value (newVal) {
                ...
                let _newVal = new Date(newVal);
                if (!isNaN(_newVal)) {
                    this.date = _newVal;
                    this.year = _newVal.getFullYear();
                    this.month = _newVal.getMonth();
                }
                if (this.showTime) {
                    this.$refs.timePicker.value = _newVal;
                }
            },
            date (val) {
                if (this.showTime) {
                    this.$refs.timePicker.date = val;
                }
            },
            format (val) {
                if (this.showTime) {
                    this.$refs.timePicker.format = val;
                }
            },
            currentView (val) {
                if (val === 'time') {
        ...
        methods: {
            ...
            handleDatePick (value) {
                if (this.selectionMode === 'day') {
                    // this.$emit('on-change', new Date(value.getTime()) + this.dateRangeMap[this.dateRange]);
                    this.$emit('on-pick', new Date(value.getTime()), this.dateRangeMap[this.dateRange]);
